...
fn py_call_attr(
    &mut self,
    _self_id: crate::heap::HeapId,
    vm: &mut VM<'_, '_, impl ResourceTracker>,    attr: &EitherStr,
    args: ArgValues,
) -> RunResult<crate::types::py_trait::AttrCallResult> {
    if attr.as_str(vm.interns) == "total_seconds" {
        args.check_zero_args("timedelta.total_seconds", vm.heap)?;
        return Ok(crate::types::py_trait::AttrCallResult::Value(Value::Float(self.total_seconds())));
    }
